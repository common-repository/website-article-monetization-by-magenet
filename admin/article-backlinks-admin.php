...
if (isset($_REQUEST['_wpnonce']) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'abp-update-settings' ) && isset( $_POST['action'] ) && $_POST['action'] === 'update_settings' ) {
    $abp_auth_key = sanitize_text_field( $_POST['abp_auth_key'] );
    $abp_auth_key = preg_replace('/[^a-zA-Z0-9-_]/', '', $abp_auth_key);

    if (strlen($abp_auth_key) == 32) {
        update_option( 'abp_auth_key', $abp_auth_key );
    }
